...
| `pcu cache`      | -                                         | Gestión de caché (estadísticas y limpieza)              |
| `pcu rollback`   | -                                         | Revertir actualizaciones de catálogo a un estado anterior |
| `pcu watch`      | -                                         | Vigilar cambios y verificar actualizaciones automáticamente |
...
pcu update       # Prompts para catálogo, formato, target, etc.
pcu security     # Prompts para formato de salida, filtro de severidad
```

```bash {{ title: 'Con Flags = Ejecución Directa' }}
...
| `cache`      | Selección de estadísticas/limpiar                   |
| `rollback`   | Selección de versión, formato                       |
| `watch`      | Intervalo, formato, opciones de notificación        |
...
```bash {{ title: 'Operaciones de Restauración' }}
pcu rollback --latest
pcu rollback --select
...
## `pcu watch` - Modo de Observación

Observar cambios en `pnpm-workspace.yaml` y verificar automáticamente dependencias desactualizadas cuando se detectan modificaciones.

<CodeGroup>

```bash {{ title: 'Observación Básica' }}
pcu watch
```

```bash {{ title: 'Observación Avanzada' }}
...
<Properties>
  ...
  <Property name="--clear" type="boolean">
    Limpiar consola antes de cada verificación
  </Property>
  <Property name="-w, --workspace" type="string">
    Ruta del directorio del workspace (predeterminado: directorio actual)
  </Property>
...
El comando watch proporciona monitoreo continuo con características inteligentes:

- **Verificación Inicial**: Ejecuta una verificación inmediatamente cuando inicia la observación
- **Debouncing**: Previene re-verificaciones rápidas cuando los archivos cambian rápidamente
- **Apagado Elegante**: Presione `Ctrl+C` para detener la observación limpiamente
- **Actualizaciones en Tiempo Real**: Muestra conteos de paquetes desactualizados a medida que se detectan

### Ejemplos de Uso

#### Modo de Observación Básico
...
Limpia la consola antes de cada verificación para una salida más limpia.
...
Espera 1 segundo después de cambios en archivos antes de ejecutar verificaciones.

#### Observar Catálogo Específico

```bash
pcu watch --catalog default --target minor
```

Observa cambios y solo verifica el catálogo default para actualizaciones minor.
...
Observa y filtra resultados a patrones de paquetes específicos.

### Consejos de Integración

El modo de observación es ideal para:

- **Flujos de Desarrollo**: Mantener las actualizaciones de dependencias visibles durante el desarrollo activo
- **Monitoreo CI/CD**: Monitorear actualizaciones en entornos de desarrollo de larga duración
- **Notificaciones de Equipo**: Combinar con herramientas de notificación para alertar sobre actualizaciones

---

## `pcu cache` - Gestión de Caché

Gestionar el caché de PCU para respuestas del registro NPM, datos del workspace y resultados de análisis AI.

<CodeGroup>

```bash {{ title: 'Mostrar Estadísticas' }}
pcu cache
pcu cache --stats
pcu cache --stats --verbose
```

```bash {{ title: 'Limpiar Caché' }}
pcu cache --clear
...
<Properties>
  <Property name="--stats" type="boolean">
    Mostrar estadísticas de caché (comportamiento predeterminado)
  </Property>
  <Property name="--clear" type="boolean">
    Limpiar todas las entradas de caché
  </Property>
  <Property name="-v, --verbose" type="boolean">
    Mostrar información detallada incluyendo descripciones y contadores de aciertos/fallos
  </Property>
  <Property name="--no-color" type="boolean">
    Deshabilitar salida a color
  </Property>
...
PCU gestiona tres cachés separados:

#### Caché de Registro

- **Propósito**: Almacena respuestas de la API del registro NPM
- **Beneficios**: Reduce solicitudes de red, consultas de versión más rápidas
- **TTL**: 1 hora (configurable)

#### Caché de Workspace

- **Propósito**: Almacena datos del sistema de archivos del workspace
- **Beneficios**: Análisis de workspace más rápido en ejecuciones repetidas
- **TTL**: 5 minutos (configurable)

#### Caché de Análisis AI

- **Propósito**: Almacena resultados de análisis impulsados por AI
...
Muestra estadísticas resumidas de todos los cachés.
...
Limpia todas las entradas de caché. Útil cuando experimentas problemas con datos obsoletos.
...
# Progreso animado arcoíris
pcu check --progress-style rainbow

# Mínimo para entornos CI/CD
...
```bash {{ title: 'Operación Normal' }}
# PCU verifica automáticamente actualizaciones
pcu check
# La salida puede incluir: "📦 PCU v0.8.0 está disponible (actual: v0.7.15)"
```

```bash {{ title: 'Deshabilitar Verificación de Actualizaciones' }}
# Deshabilitar para un solo comando
pcu check --no-update-check

# Deshabilitar globalmente vía entorno
...
```bash
# PCU intenta en orden: npm → pnpm → yarn
...
```bash
pcu check
# ✨ PCU v0.8.0 está disponible (actual: v0.7.15)
...
```bash
pcu security
# 🔒 ACTUALIZACIÓN DE SEGURIDAD: PCU v0.7.16 contiene correcciones de seguridad (actual: v0.7.15)
...
pcu check
# 🧪 PCU v0.8.0-beta.1 está disponible para pruebas (actual: v0.7.15)
...
```bash {{ title: 'Verificar Estado de Actualización' }}
# Verificar versión actual y actualizaciones disponibles
pcu --version
pcu -V
```

```bash {{ title: 'Actualización Manual' }}
# Actualizar vía npm
npm install -g pcu@latest
...
```bash
# Si la verificación de actualización falla por problemas de red
PCU_UPDATE_TIMEOUT=10000 pcu check

# Forzar verificación de actualización incluso en CI
...
```bash
# Limpiar caché de verificación de actualización
...
```bash {{ title: 'Estado de Caché' }}
# Ver estadísticas de caché (vía modo verbose)
pcu check --verbose
# Muestra: Aciertos de caché: 85%, Tamaño: 15.2MB, Entradas: 342
...
# Limpiar tipo de caché específico
rm -rf ~/.pcu/cache/registry
rm -rf ~/.pcu/cache/workspace

# Deshabilitar caché para un solo comando
PCU_CACHE_ENABLED=false pcu check
```
