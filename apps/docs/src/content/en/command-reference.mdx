...
| `pcu cache`     | -                   | Manage PCU cache (registry, workspace, AI)      |
| `pcu rollback`  | -                   | Rollback catalog updates to a previous state    |
| `pcu watch`     | -                   | Watch for changes and auto-check updates        |
...
```bash {{ title: 'No Flags = Interactive Prompts' }}
# Running without flags triggers interactive prompts
pcu check        # Prompts for format, target, filter options
pcu update       # Prompts for catalog, format, target, backup, etc.
pcu security     # Prompts for output format, severity filter
pcu analyze      # Prompts for catalog selection and package
pcu theme        # Prompts for theme selection
```

```bash {{ title: 'With Flags = Direct Execution' }}
# Providing flags runs directly without prompts
...
Hybrid Mode is available for all 10 main commands:
...
| `pcu cache`    | statistics view, clear options                           |
| `pcu rollback` | backup selection, confirmation                           |
| `pcu watch`    | catalog, format, target, debounce, clear options         |
...
```bash {{ title: 'Rollback Operations' }}
pcu rollback --latest
pcu rollback --select
...
## `pcu watch` - Watch Mode

Watch for changes in `pnpm-workspace.yaml` and automatically check for outdated dependencies when modifications are detected.

<CodeGroup>

```bash {{ title: 'Basic Watch' }}
pcu watch
```

```bash {{ title: 'Advanced Watch' }}
...
<Properties>
  ...
  <Property name="-t, --target" type="enum" default="latest">
    Update target: latest, minor, patch
  </Property>
  <Property name="--prerelease" type="boolean">
    Include prerelease versions
  </Property>
  ...
  <Property name="--debounce" type="number" default="300">
    Debounce delay in milliseconds before checking
  </Property>
  <Property name="--clear" type="boolean">
    Clear console before each check
  </Property>
  <Property name="-w, --workspace" type="string">
    Workspace directory path (default: current directory)
  </Property>
...
The watch command provides continuous monitoring with smart features:

- **Initial Check**: Runs a check immediately when watch starts
- **Debouncing**: Prevents rapid re-checks when files change quickly
- **Graceful Shutdown**: Press `Ctrl+C` to stop watching cleanly
- **Real-time Updates**: Shows outdated package counts as they're detected
...
Watches the current directory's `pnpm-workspace.yaml` for changes.

#### Watch with Console Clear

```bash
pcu watch --clear
```

Clears the console before each check for a cleaner output.
...
Waits 1 second after file changes before running checks.

#### Watch Specific Catalog

```bash
pcu watch --catalog default --target minor
```

Watches for changes and only checks the default catalog for minor updates.
...
Watches and filters results to specific package patterns.

### Integration Tips

Watch mode is ideal for:

- **Development Workflows**: Keep dependency updates visible during active development
- **CI/CD Monitoring**: Monitor for updates in long-running development environments
- **Team Notifications**: Combine with notification tools to alert about updates

---

## `pcu cache` - Cache Management

Manage PCU cache for NPM registry responses, workspace data, and AI analysis results.

<CodeGroup>

```bash {{ title: 'View Statistics' }}
pcu cache
pcu cache --stats
...
<Properties>
  <Property name="--stats" type="boolean">
    Show cache statistics (default behavior)
  </Property>
  <Property name="--clear" type="boolean">
    Clear all cache entries
  </Property>
  <Property name="-v, --verbose" type="boolean">
    Show detailed information including descriptions and hit/miss counts
  </Property>
...
PCU maintains three separate caches:

#### Registry Cache

- **Purpose**: Stores NPM registry API responses
- **Benefits**: Reduces network requests, faster version lookups
- **TTL**: 1 hour (configurable)

#### Workspace Cache

- **Purpose**: Stores workspace file system data
- **Benefits**: Faster workspace analysis on repeat runs
- **TTL**: 5 minutes (configurable)

#### AI Analysis Cache

- **Purpose**: Stores AI-powered analysis results
- **Benefits**: Avoids redundant AI API calls, consistent results
- **TTL**: 24 hours (configurable)

### Statistics Output

The stats command shows for each cache:
...
Shows summary statistics for all caches.
...
Shows detailed statistics including descriptions and hit/miss breakdown.
...
Clears all cache entries. Useful when experiencing stale data issues.
...
Clears all caches with detailed output showing each cache being cleared.
