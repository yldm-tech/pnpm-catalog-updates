...
| `pcu cache`     | -                                         | 缓存管理（统计和清除）          |
| `pcu rollback`  | -                                         | 将目录更新回滚到之前的状态      |
| `pcu watch`     | -                                         | 监视更改并自动检查更新          |
...
监视 `pnpm-workspace.yaml` 的变化，当检测到修改时自动检查过时的依赖项。
...
watch 命令提供具有智能功能的持续监控：

- **初始检查**：监视启动时立即运行检查
- **防抖**：防止文件快速变化时的重复检查
- **优雅关闭**：按 `Ctrl+C` 干净地停止监视
- **实时更新**：显示检测到的过时包数量
...
监视当前目录的 `pnpm-workspace.yaml` 变化。
...
- **开发工作流**：在活跃开发期间保持依赖更新可见
- **CI/CD 监控**：在长时间运行的开发环境中监控更新
- **团队通知**：与通知工具结合以提醒更新
...
管理 PCU 缓存，包括 NPM 注册表响应、工作空间数据和 AI 分析结果。
...
- **用途**：存储 AI 驱动的分析结果
- **优点**：避免重复的 AI API 调用，结果一致
- **TTL**：24 小时（可配置）

#### 缓存特性

- **自动过期**：缓存条目根据 TTL 设置自动过期
- **LRU 淘汰**：最少使用的项目首先被移除
- **大小监控**：超过大小限制时自动淘汰
- **磁盘持久化**：注册表缓存支持磁盘持久化
...
清除所有缓存条目。如果遇到陈旧数据问题，可以使用此命令强制清除。
