...
- 🐛 **Issues**: Report bugs via [GitHub Issues](https://github.com/yldm-tech/pnpm-catalog-updates/issues)
- 💬 **Discussions**: Ask questions in [GitHub Discussions](https://github.com/yldm-tech/pnpm-catalog-updates/discussions)
...
├── version.ts           // Semantic version handling
├── workspacePath.ts     // Path validation
├── workspaceConfig.ts   // Configuration object
...
├── catalogUpdateService.ts  // Main orchestration
...
├── repositories/
│   └── fileWorkspaceRepository.ts  // File system persistence
├── cache/
│   └── cache.ts                # Advanced caching system
└── utils/
    └── npmrcParser.ts          # Configuration parsing
...
├── checkCommand.ts     // Check command implementation
├── updateCommand.ts    // Update command with validation
├── analyzeCommand.ts   // Impact analysis command
├── workspaceCommand.ts # Workspace management
├── themeCommand.ts     # Theme configuration
├── securityCommand.ts  # Security scanning
...
// apps/cli/src/cli/
├── validators/
│   └── commandValidator.ts    # Input validation with suggestions
├── interactive/
│   └── interactivePrompts.ts  # Smart prompts with auto-completion
├── themes/
│   └── colorTheme.ts          # Comprehensive theming system
└── formatters/
    └── outputFormatter.ts     # Multiple output formats
...
├── packages/utils/src/**/__tests__/    # Utility tests
