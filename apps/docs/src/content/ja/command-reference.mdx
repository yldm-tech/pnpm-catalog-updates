...
| `pcu cache`     | -                                         | キャッシュ管理（統計とクリア）         |
| `pcu rollback`  | -                                         | カタログ更新を以前の状態にロールバック |
| `pcu watch`     | -                                         | 変更を監視して自動的に更新をチェック   |
...
ハイブリッドモードは 10 個すべての主要コマンドで利用可能です：
...
| `pcu cache`    | 統計表示、クリアオプション                               |
| `pcu rollback` | バックアップ選択、確認                                   |
| `pcu watch`    | カタログ、フォーマット、ターゲット、デバウンス、クリアオプション |
...
`pnpm-workspace.yaml` の変更を監視し、変更が検出されたときに自動的に古い依存関係をチェックします。
...
watch コマンドはスマートな機能を備えた継続的な監視を提供します：

- **初期チェック**：ウォッチ開始時にすぐにチェックを実行
- **デバウンス**：ファイルが急速に変更されるときの再チェックを防止
- **グレースフルシャットダウン**：`Ctrl+C` を押してクリーンに監視を停止
- **リアルタイム更新**：検出された古いパッケージ数を表示
...
現在のディレクトリの `pnpm-workspace.yaml` の変更を監視します。
...
各チェック前にコンソールをクリアして、より見やすい出力を提供します。
...
変更を監視し、default カタログのマイナー更新のみをチェックします。
...
監視し、特定のパッケージパターンに結果をフィルタリングします。

### 統合のヒント

ウォッチモードは以下に最適です：

- **開発ワークフロー**：アクティブな開発中に依存関係の更新を可視化
- **CI/CD 監視**：長時間稼働する開発環境での更新を監視
- **チーム通知**：通知ツールと組み合わせて更新をアラート

---

## `pcu cache` - キャッシュ管理

NPM レジストリ応答、ワークスペースデータ、AI 分析結果のための PCU キャッシュを管理します。
...
PCU は 3 つの別々のキャッシュを管理します：

#### レジストリキャッシュ

- **目的**: NPM レジストリ API 応答を保存
- **利点**: ネットワークリクエストを削減、より高速なバージョンクエリ
- **TTL**: 1 時間（設定可能）

#### ワークスペースキャッシュ

- **目的**: ワークスペースファイルシステムデータを保存
- **利点**: 繰り返し実行時のより高速なワークスペース分析
- **TTL**: 5 分（設定可能）

#### AI 分析キャッシュ

- **目的**: AI 駆動の分析結果を保存
- **利点**: 冗長な AI API 呼び出しを回避、一貫した結果
- **TTL**: 24 時間（設定可能）
...
すべてのキャッシュエントリをクリアします。古いデータの問題が発生した場合に便利です。
